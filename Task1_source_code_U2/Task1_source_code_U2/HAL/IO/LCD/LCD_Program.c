...
static void LCD_voidWriteBitsToSend(u8 copy_u8Byte)
{
	...
	DIO_SetPinValue(D7_PORT,D7_PIN, GET_BIT(copy_u8Byte,PIN7) );
	DIO_SetPinValue(D6_PORT,D6_PIN, GET_BIT(copy_u8Byte,PIN6) );
	DIO_SetPinValue(D5_PORT,D5_PIN, GET_BIT(copy_u8Byte,PIN5) );
	DIO_SetPinValue(D4_PORT,D4_PIN, GET_BIT(copy_u8Byte,PIN4) );

	/* open EN--> make it high --- then after 2m close EN -->make it low*/
	DIO_SetPinValue(EN_PORT,EN_PIN, DIO_HIGH);
	_delay_ms(2);
	DIO_SetPinValue(EN_PORT,EN_PIN, DIO_LOW);


	/*prepare 4 bits LSB to send */
	DIO_SetPinValue(D7_PORT,D7_PIN, GET_BIT(copy_u8Byte,PIN3) );
	DIO_SetPinValue(D6_PORT,D6_PIN, GET_BIT(copy_u8Byte,PIN2) );
	DIO_SetPinValue(D5_PORT,D5_PIN, GET_BIT(copy_u8Byte,PIN1) );
	DIO_SetPinValue(D4_PORT,D4_PIN, GET_BIT(copy_u8Byte,PIN0) );
